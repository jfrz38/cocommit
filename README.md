...
`cocommit` is a small keyboard-driven terminal UI for reviewing a concise snapshot of already staged changes and creating Conventional Commit headers. It previews the message as you edit it, then delegates the commit to your installed Git CLI.
...
- Review staged file states and aggregate line statistics without leaving the composer.
...
The expanded layout shows a bounded, scrollable staged-change snapshot with file count, `A/M/D/R` states, insertions, deletions, and binary-file count. Focus `Staged changes` with `Tab`; `Up`/`Down` scroll its list and `Home`/`End` move to its first/last item. The snapshot is captured before the form opens, so it does not claim to reflect index changes made while editing. The layout switches to a compact, vertically scrolling view in smaller terminals. Below `30x8`, it displays a resize instruction instead of the form.
...
Before opening the form, cocommit verifies that Git is available, the current directory is inside a usable working tree, and staged changes exist. It then reads a NUL-delimited, read-only summary of the index; unusual names are safely escaped for the terminal. This works from subdirectories and linked worktrees. Git remains authoritative after that check, so a changed index can still cause the final commit to fail.
...
- cocommit does not stage or unstage files, render full diffs or history, manage branches, push, or create pull requests.
