...
Message policy records configurable allowed types, scope suggestions, subject policies, and decimal issue identifier prefix/rendering style. Scope rendering remains the Conventional Commits `()` syntax only. ADR 0012 defines policy enforcement through the single domain renderer and validator.
